...
module.exports = {
  query : function(){
    if(inObj.isValid){return inObj.query;}
  },
  explain : function(){
    if(inObj.isValid){return inObj.qryExplain;}
  },
  init : function( let, crit, method,len ){
 	   inObj = new lexSearch(let, crit,method, len);
	   if(inObj.isValid){
		 return true;  
	   }else{
		 return inObj.errortxt; 
	   }
  }
};
function lexSearch(let, crit, method,len){
    ...
    if(this.isValid){
       if (this.getLength(this.inLetter) + this.getLength(this.inCriteria)<3){
        ...
        if (this.isValid && this.getLength(this.inLetter)>10){
             this.errortxt='maximun number of input letters exceeded (10).'; 
             this.isValid= false;
        }
        if(this.isValid){
   ...
   //create query based on input
    if(this.isValid){
        ...
        if(this.letter){
                ...
                for(x in __wordPool){s=s+__wordPool[x]+"','";}
            ...
            switch (this.method) {
                    ...
                    case 'con':  //contains
                        ...
                        if(!crit1) {crit1="";exp1="";}else{crit1=" AND word like '%"+crit1+"%' ";}
                    break;
                    case 'sta': //start with
                        crit1=this.criteria.replace(/[*]/g,"");
...
var __wordPool=new Array;
        ...
        function getTheWordTreeExtra(word, extra){
            var y=word+"", nw="", l="", wA, w, e;
            w=y.split("").sort().join("");

            if(extra) {
                if(w.length +extra.length <3 || w.length>10) return false; //min size
                nw=w.concat(extra).split("").sort().join("");
            }else{
                if(w.length<3 || w.length>10) return false; //min size
				nw=w;
            }
